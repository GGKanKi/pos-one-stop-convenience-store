      ...
      if (data.success) {
        ...
        if (data.user.role === 'admin') {
          ...
          if (data.user.hasStaffId) {
            // NEW: If staff already has an ID, navigate to LoginId for PIN entry
            navigate("/loginid", { state: { email: email } }); 
          } else {
        ...
        <div className="relative z-10 flex flex-col justify-between w-full text-white p-6 md:p-10 lg:p-16">
          <div>
            <div className="flex items-center gap-3 mb-6 md:mb-8">
              ...
              <div>
                <h1 className="text-xl md:text-2xl lg:text-3xl font-bold">One Stop</h1>
                <p className="text-blue-200 text-sm">Convenience Store</p>
              </div>
            </div>

            <div className="max-w-md">
              <h2 className="text-2xl md:text-4xl lg:text-5xl leading-tight mb-4 md:mb-6 font-bold">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 max-w-md">
            <div className="flex flex-col items-center gap-3 p-4 md:p-6 bg-white/10 rounded-2xl backdrop-blur-sm border border-white/10">
      ...
      <div className="flex flex-1 items-center justify-center bg-gradient-to-br from-blue-900 via-blue-800 to-blue-950 px-6 py-12">
        <div className="w-full max-w-md">
          <form
            ...
            className="flex flex-col gap-6 p-6 rounded-xl bg-white/10 backdrop-blur-md border border-white/20 shadow-xl text-white"
          >
            {errorMsg && (
              <div className="bg-red-500/20 border border-red-400 text-red-200 px-4 py-2 rounded-lg text-sm text-center">
                {errorMsg}
              </div>
            )}
            ...
            <input
              type="email"
              placeholder="Email"
              className="px-4 py-3 rounded-lg bg-white/10 text-white placeholder-gray-400 border border-white/20 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
              value={email}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg transition-all shadow-lg shadow-blue-500/30 disabled:opacity-50"
            >
              {loading ? "Checking..." : "Sign In"}
            </button>

            <Link to="/forgot-password" title="Forgot Password" className="text-blue-300 hover:text-blue-400 text-sm underline text-center">
              Forgot password?
            </Link>

            <p className="text-sm text-gray-300 text-center">
              Don't have an account?{" "}
              <Link to="/signup" className="underline text-blue-300 hover:text-blue-400 font-medium">
                Sign up here
              </Link>
            </p>
          </form>
